...
<template>
  <div class="relative w-full">
    ...
    <div class="relative">
      <input v-model="inputValue" type="text" placeholder="Rechercher une ville" :class="[
    ...
    <ul v-if="search.results.length"
      class="absolute z-10 w-full mt-1 bg-white border border-gray-200 rounded-md shadow-lg max-h-48 overflow-auto">
      <li v-for="city in search.results" :key="`${city.name}-${city.lat}-${city.lng}`" @click="selectCity(city)"
